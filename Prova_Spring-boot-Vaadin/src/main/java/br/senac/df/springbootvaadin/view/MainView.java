...
public class MainView extends VerticalLayout {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5110944319994902290L;
	private final AlunoRepositorio repo;
	final Grid<Aluno> grid;
	...
	private String MAX_WIDTH = "770px";
	private String BUTTON_WIDTH = "123px";

	public MainView(AlunoRepositorio repo) {
		this.repo = repo;
		this.grid = new Grid<>(Aluno.class, false);
		grid.addColumn(Aluno::getId).setHeader("ID").setSortable(true).setWidth("20px");
		grid.addColumn(Aluno::getNome).setHeader("Nome").setSortable(true);
		grid.addColumn(Aluno::getSobrenome).setHeader("Sobrenome").setSortable(true);
		grid.addColumn(Aluno::getEmail).setHeader("Email").setSortable(true);
		grid.addColumn(Aluno::getIdade).setHeader("Idade").setSortable(true);
		...
		fieldsLayout.add(nome, sobrenome, email, idade);
	...
	private void addButtonsActionListeners() {
		...
		grid.addSelectionListener(selected -> {
			if (selected.getAllSelectedItems().isEmpty()) {
				...
				Aluno selectedCustomer = selected.getFirstSelectedItem().get();
				...
				idade.setValue(selectedCustomer.getIdade());
